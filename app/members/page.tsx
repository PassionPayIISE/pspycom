        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-lg font-semibold">
              {member.name?.trim() ? member.name : member.email ?? "이름 없음"}
            </h2>
          ...
          <div className="mt-3 space-y-1 text-sm text-gray-600">
            ...
            <p>
              <span className="font-medium text-gray-800">학번:</span>{" "}
              {member.student_id ?? "-"}
            </p>
            <p>
...
export default async function AdminMembersPage() {
  ...
  const { data: profiles, error } = await supabase
    .from("profiles")
    .select("id, email, name, student_id, role, approved, created_at, updated_at")
  ...
  const approvedMembers = allProfiles.filter(
    (p) => p.role === "member" && p.approved === true
  );
  const adminMembers = allProfiles.filter(
    (p) => p.role === "admin" && p.approved === true
  );
